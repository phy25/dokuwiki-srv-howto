FROM alpine:edge
MAINTAINER Phy <dockerfile@phy25.com>
# Originally from Ilya Stepanov <dev@ilyastepanov.com>

RUN apk --no-cache --repository http://dl-cdn.alpinelinux.org/alpine/edge/main \
        --repository http://dl-cdn.alpinelinux.org/alpine/edge/community/ add \
        php7 php7-fpm php7-gd php7-session php7-xml php7-dom php7-curl php7-openssl php7-json php7-mbstring php7-iconv php7-phar nginx supervisor curl unzip bash

RUN adduser -u 82 -D -S -G www-data www-data
# www-data already exists, added by nginx

RUN mkdir -p /run/nginx && \
    mkdir -p /var/dokuwiki-storage/ && \
    cd /var/www

ADD index.html /var/www/index.html
ADD nginx.conf /etc/nginx/nginx.conf
ADD supervisord.conf /etc/supervisord.conf
ADD start.sh /start.sh

RUN echo "cgi.fix_pathinfo = 0;" >> /etc/php7/php-fpm.ini && \
    sed -i -e "s|;daemonize\s*=\s*yes|daemonize = no|g" /etc/php7/php-fpm.conf && \
    sed -i -e "s|listen\s*=\s*127\.0\.0\.1:9000|listen = /var/run/php-fpm7.sock|g" /etc/php7/php-fpm.d/www.conf && \
    sed -i -e "s|;listen\.owner\s*=\s*nobody|listen.owner = www-data|g" /etc/php7/php-fpm.d/www.conf && \
    sed -i -e "s|;listen\.group\s*=\s*nobody|listen.group = www-data|g" /etc/php7/php-fpm.d/www.conf && \
    sed -i -e "s|;listen\.mode\s*=\s*|listen.mode = |g" /etc/php7/php-fpm.d/www.conf && \
    chmod +x /start.sh

EXPOSE 80
VOLUME ["/var/www"]

CMD /start.sh